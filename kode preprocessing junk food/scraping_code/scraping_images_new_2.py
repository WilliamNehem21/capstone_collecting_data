...
# -*- coding: utf-8 -*-
"""
Created on Thu May 18 18:41:07 2023
...
def get_original_images(folder_name, menu, page):
    ...
    for index, (metadata, thumbnail, original) in enumerate(zip(soup.select('.isv-r.PNCib.MSM1fd.BUooTd'), thumbnails, full_res_images), start=1):
        ...
        try:
            new_path = "/Users/williamnehemia/Documents/Bangkit-Course/download_image/clean_menu_3/" + folder_name + "/"
            ...
            try:
                ...
                if (isi_before == None):
                    ...
                    urllib.request.urlretrieve(original, f'{new_path}{menu}{page}.{index}.jpg')
                else:
...
for index, row in df.iterrows():
    if(row['Menu'] == 'Dough Ball Formagi' and row['Company'] == 'Pizza Marzano'):
        indexStart = index
        break
print(indexStart)
indexStart = 0
...
for item in range(0,6):
        ...
        if(index >= indexStart):
            ...
            if (row['Menu'] in list_judul_ikut):
                ...
                params = {
                    ...
                    "hl": "en",                   # language of the search
                    "gl": "us",                   # country where search comes from
                    "ijn": str(item)                    # page number
                }
                html = requests.get("https://www.google.com/search", params=params, headers=headers, timeout=30)
                soup = BeautifulSoup(html.text, "lxml")
                
                get_suggested_search_data(params)
                get_original_images(row['Jenis'], row['Menu'], str(item) )
    ...
    for item_judul in list_judul_ikut :
        params = {
            "q": item_judul, # search query
            "tbm": "isch",                # image results
            "hl": "en",                   # language of the search
            "gl": "us",                   # country where search comes from
            "ijn": str(item)                   # page number
        ...
        get_suggested_search_data(params)
        get_original_images(item_judul, item_judul, str(item) )
        
    for item_judul in list_judul_ikut :
        params = {
            "q": item_judul + " food", # search query
            "tbm": "isch",                # image results
            "hl": "en",                   # language of the search
            "gl": "us",                   # country where search comes from
            "ijn": str(item)                 # page number
